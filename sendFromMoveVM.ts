import { getConfigConnectionsFromChainType } from '@layerzerolabs/devtools-move'
import { ChainType } from '@layerzerolabs/lz-definitions'
async function main() {
    const connectionsToWire = await getConfigConnectionsFromChainType('from', ChainType.EVM, 'move.layerzero.config.ts')
    console.log(connectionsToWire)
}
main().catch((err) => {
    console.error(err)
})
